...
VERSION := $(shell cargo metadata --format-version 1 | jq -r '.packages[] | select(.name=="stackable-hdfs-operator") | .version')
...
docker-build:
	docker build --force-rm --build-arg VERSION=${VERSION} -t "docker.stackable.tech/stackable/hdfs-operator:${VERSION}" -f docker/Dockerfile .
...
chart-clean:
	...
	rm -rf deploy/helm/hdfs-operator/crds

version:
	yq eval -i '.version = strenv(VERSION) | .appVersion = strenv(VERSION)' /dev/stdin < deploy/helm/hdfs-operator/Chart.yaml
	yq eval -i '.version = strenv(DOCS_VERSION) | .prerelease = strenv(IS_NIGHTLY) != ""' /dev/stdin < docs/antora.yml

config:
	if [ -d "deploy/config-spec/" ]; then\
		mkdir -p deploy/helm/hdfs-operator/configs;\
		cp -r deploy/config-spec/* deploy/helm/hdfs-operator/configs;\
	fi

crds:
	mkdir -p deploy/helm/hdfs-operator/crds
	cargo run --bin stackable-hdfs-operator -- crd | yq eval '.metadata.annotations["helm.sh/resource-policy"]="keep"' - > deploy/helm/hdfs-operator/crds/crds.yaml
