...
We provide a more representative rego rule in our integration tests and in the afore-mentioned hdfs-utils repository.
Details can be found below in the <<_fine_granular_rego_rule_, fine-granular rego rule>> section.
...
Each operation has an implicit action-level attribute e.g. `create` requires at least read-write permissions. This action attribute is then checked against the permissions assigned to the user by an ACL list and the operation is permitted if this check is fulfilled.
...
# Access is turned off by default
default allow := false
default matches_identity(identity) := false

# Access is checked in order of increasing specificity (i.e. identity first).
# This allows access to be denied as "early" as possible.
allow if {
    some acl in acls
...
# The operation and its implicit action is checked against an ACL
...
In hdfs-utils there is also a https://github.com/stackabletech/hdfs-utils/blob/main/rego/hdfs_test.rego[test file] to verify the rules, where different asserts are applied to the rules. Take the test case below as an example:
