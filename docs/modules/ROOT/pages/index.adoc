...
In the custom resource you can specify the number of replicas per role group (name node, data node or journal node). A minimal working configuration requires:

* 2 name nodes (HA)
* 1 journal node
* 1 data node (should match at least the `dfsReplication` factor)
