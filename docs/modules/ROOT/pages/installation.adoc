...
There are three ways to run the HDFS Operator:
...
----
sudo apt-get update
sudo apt-get install stackable-hdfs-operator
----
...
[source,bash]
----
sudo yum install stackable-hdfs-operator
----
...
[source]
----
docker.stackable.tech/stackable/hdfs-operator
----

To run it straight from Docker you can use this command:
[source,bash]
----
docker run -e HDFS_OPERATOR_LOG=trace -e KUBECONFIG=/root/.kube/k3s.yaml --mount type=bind,source="$HOME/.kube/k3s.yaml",target="/root/.kube/k3s.yaml" -it docker.stackable.tech/stackable/hdfs-operator:latest
----
